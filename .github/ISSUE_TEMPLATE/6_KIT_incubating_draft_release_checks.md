---
name: KIT Incubating Draft Release Checklist
about: Checklist for a KIT in Incubating Draft stage
title: '[KIT-DRAFT] <KIT Name> - Incubating Draft Release Checklist'
labels: 'kit, incubating, draft'
assignees: ''
---

# KIT Incubating Draft Release Checklist
...
- [ ] **Business Processes** *(recommended)* - Business workflows, process flows, and BPMN diagrams (may not be relevant for Network Services)
- [ ] **Tutorials / Videos** *(optional)* - Step-by-step learning materials, video demonstrations, and hands-on exercises
- [ ] **Whitepaper** *(optional)* - In-depth technical or business analysis, research findings, and use case studies

## Development View

- [ ] **Architecture Overview** *(recommended)* - High-level system design, C4 model diagrams, and integration points
- [ ] **Component/Sequence Diagrams** *(recommended)* - Visual representation using UML or PlantUML showing component interactions
- [ ] **Standards** *(recommended)* - Industry and technical standards compliance with links to specifications
- [ ] **API-Specification / Protocols** *(recommended)* - Open API Specifications 
- [ ] **Access & Usage Policies** *(recommended)* - If DSP is used (via Connector), the policies which are required
- [ ] **Logic / Schema** *(recommended)* - Business logic documentation, database schemas (ERD diagrams), and data flow diagrams
- [ ] **Semantic Models / Data Structures** *(recommended)* - Formal data model definitions (SAMM models), JSON Schema, and relationships/constraints
- [ ] **Test Cases** *(recommended)* - Unit, integration, and E2E test scenarios with coverage requirements (>80%)

## Operation View

- [ ] **Deployment Guide** *(recommended)* - Step-by-step installation with Docker, Kubernetes, Helm charts, prerequisites, and system requirements
- [ ] **Operation/Monitoring Guidelines** *(optional)* - How to run and maintain with Prometheus metrics, logging, and key health indicators
- [ ] **Security Guidelines** *(optional)* - Security best practices, OAuth2/OIDC setup, and data encryption requirements

## Success Stories

- [ ] **Reference Implementations** *(optional)* - Working examples with GitHub repository links, real deployments (COTS or open source)

## Documentation

- [ ] **Extra Documentation / Links** *(recommended)* - Additional resources, FAQs, community channels, related KITs, and integrations
- [ ] **Sample Data** *(optional)* - Example datasets in JSON, CSV formats (anonymized and privacy-compliant)

## Industry Extensions

- [ ] **At least one Industry Extension** *(recommended)* - Industry-specific adaptations, Catena-X standards compliance, and links to published standards
